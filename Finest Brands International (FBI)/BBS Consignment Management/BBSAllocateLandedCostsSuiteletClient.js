								...
								if(totalLineWeight != NaN && totalLineWeight != 0)
									{
										...
										for (var linenum = 1; linenum <= lineCount; linenum++) 
										{
											...
											if (lineCurrency != '' && lcCurrency != '')
												{
													var exchangeRate = Number(1.0);
													
													if(lineCurrency != lcCurrency)
														{
															exchangeRate = nlapiExchangeRate(lineCurrency, lcCurrency);
														}
													
													lineLandedCostInCurrency = (lineLandedCost * exchangeRate).toFixed(2);
												}
									...
									if (lineCurrency != '' && lcCurrency != '')
										{
											...
											lineValue = lineValue * exchangeRate;
										}
