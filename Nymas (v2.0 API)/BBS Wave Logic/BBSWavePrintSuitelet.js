									...
									if(picktaskSearchObj != null && picktaskSearchObj.length > 0)
										{
													...
													if(parentItems[parentItem])
														{
															...
															//Unique key is sales order id + '|' + sales order line number
															//
															var uniqueKey = padding_left(salesOrderId, '0', 8) + '|' + padding_left(salesOrderLine, '0', 8);
															
															//Add a new header object if required
															//
															if(!outputObj.hasOwnProperty(uniqueKey))
																{
																	//Go to the original order line to find the quantity required of the DocM kit
																	//
																	var lineQuantity = Number(1);
																	...
																	if(salesorderSearchObj != null && salesorderSearchObj.length == 1)
																		{
																			lineQuantity = Number(salesorderSearchObj[0].getValue({name: "quantity"}));
																		}
																	...
																	//
																	outputObj[uniqueKey].product 		= parentItemDesc;			//DocM product
																	outputObj[uniqueKey].waveNumber 	= waveNumber;				//Wave number
																	outputObj[uniqueKey].orderNumber	= salesOrderNumber;			//Sales order no
																	outputObj[uniqueKey].customer		= salesCustomer;			//Customer
																	outputObj[uniqueKey].releaseDate	= releasedDate;				//Wave release date
																	outputObj[uniqueKey].description	= parentDescs[parentItem];	//DocM product description
																	outputObj[uniqueKey].location		= location;					//Location (warehouse)
																	outputObj[uniqueKey].quantity		= lineQuantity;				//Order quantity of DocM product
											...
											for ( var uniqueKey in outputObj) 
												{
													var orderedQuantity = outputObj[uniqueKey].quantity;
													
													for (var loopCounter = 0; loopCounter < orderedQuantity; loopCounter++) 
														{
															xmlStr += '	<pdf>';
															xmlStr += '	<head>';
															...
															xmlStr += '	      			<tr>';
															xmlStr += '	      				<td align="left" ><b>Quantity</b></td>';
															xmlStr += '	      				<td align="left" colspan="2">' + (loopCounter + 1).toFixed(0) + ' of ' + orderedQuantity.toFixed(0) + '</td>';
															xmlStr += '                      	<td align="right">&nbsp;</td>';
															xmlStr += '	      				<td align="left" >&nbsp;</td>';
															xmlStr += '	      			</tr>';
															...
															xmlStr += '	</style>';
															xmlStr += '	</head>';
															xmlStr += '	<body header="nlheader" header-height="170pt" footer="nlfooter" footer-height="20pt" padding="0.5in 0.5in 0.5in 0.5in" size="A4">';
															...
															for (var int3 = 0; int3 < outputObj[uniqueKey].lines.length; int3++) 
																{
																	...
																	xmlStr += '<td align="center" colspan="1">' + xml.escape({xmlText: (outputObj[uniqueKey].lines[int3].quantity / orderedQuantity).toFixed(2)}) + '</td>';
