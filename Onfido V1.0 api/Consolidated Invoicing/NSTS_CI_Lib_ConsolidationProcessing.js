...
function processGeneratePDF(ciId, recCI, ciConfig, arrPDFLog, arrEmailLog, arrFaxLog, arrErrors, arrAREmails)
{
    ...
    if (objCITemplate)
    {
        ...
        try
        {
            ...
            //Add code to see if custom emailing is enabled, if so use the field "custentity_bbs_invoice_email" on the customer
            //
            if(ciConfig.useCustomEmailing =='T')
            	{
            		stEmail = [objCustomer.getFieldValue("custentity_bbs_invoice_email")];
            		log("debug", "Using custom emailing to " + stEmail);
            	}
            ...
            log("debug", stLogTitle,"arrCi2:" + JSON.stringify(arrCi2));
