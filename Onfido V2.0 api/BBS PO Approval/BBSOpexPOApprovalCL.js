...
function(search) {
    ...
    function fieldChanged(scriptContext) {
    	...
    	// check that the 'Capex Type' field has been changed
    	if (scriptContext.fieldId == 'custbody_bbs_cap_type')
    		{
    			// get the current record so it can be manipulated
				var currentRecord = scriptContext.currentRecord;
		    		
		    	// get the value of the 'Purchase Order Type' field
		    	var poType = currentRecord.getValue({
		    		fieldId: 'custbody_po_type'
		    	});
		    			
		    	// check if the poType variable returns 1 (Capex)
		    	if (poType == 1)
		    		{
