 ...
 */
define(['N/runtime', 'N/search', 'N/format','N/record','N/xml','N/render','N/file','N/email',
...
function(runtime, search, format, record, xml, render, file, email, BBSObjects, BBSCommon, BBSCarrierGFS) 
{
     ...
     * @Since 2015.2
     */
    function executeEodCommit(scriptContext) 
	    {
	    	...
	    	var emailRecipient 		= currentScript.getParameter({name: 'custscript_bbs_shipment_eod_receiver'});
	    	
    		var today 				= new Date();
    		var dateString 			= today.format('Ymd');
    		var emailAttachments 	= [];
    		
    		//Search for enabled integrations
    		//
			...
			if(customrecord_bbs_carrier_service_codesSearchObj != null && customrecord_bbs_carrier_service_codesSearchObj.length > 0)
				{
					for (var int = 0; int < customrecord_bbs_carrier_service_codesSearchObj.length; int++) 
						{
							var integratorId 	= customrecord_bbs_carrier_service_codesSearchObj[int].getValue({name: "custrecord_bbs_primary_carrier",join: "CUSTRECORD_BBS_SC_CARRIER"})
							var integratorName 	= customrecord_bbs_carrier_service_codesSearchObj[int].getText({name: "custrecord_bbs_primary_carrier",join: "CUSTRECORD_BBS_SC_CARRIER"})
							var carrierName 	= customrecord_bbs_carrier_service_codesSearchObj[int].getValue({name: "custrecord_bbs_carrier_code",join: "CUSTRECORD_BBS_SC_CARRIER"})
							...
							if(integrationObject)
								{
									...
									//Create the empty manifest file object
									//
									...
									switch(integratorName)
		    							{
				    						case 'GFS':
		    									...
		    									if(commitShipmentResponse.status == 'SUCCESS')
		    										{
		    											...
		    											try
		    												{
				    											manifestFile = file.create({
				    																			name:			'Manifest_' + carrierName + '_' + dateString +'.pdf',
				    																			fileType:		file.Type.PDF,
				    																			contents:		commitShipmentResponse.manifest,
				    																			description:	'Manifest File ' + carrierName + ' ' + dateString,
				    																			folder:			fileCabinetFolder,
				    																			isOnline:		true
				    																			});
				    											
				    											manifestFileId = manifestFile.save();
		    												}
		    											catch(err)
			    											{
			    												log.error({
			    															title: 		'Unable to create manifest PDF file',
			    															details: 	err
			    															});
			    											}
		    										}
		    									else
		    										{
		    											...
		    											try
		    												{
				    											manifestFile = render.xmlToPdf({xmlString: xml});
				    											
				    											manifestFile.description 	= 'Manifest File ' + carrierName + ' ' + dateString;
				    											manifestFile.name 			= 'Manifest_' + carrierName + '_' + dateString +'.pdf';
				    											manifestFile.folder 		= fileCabinetFolder;
				    											manifestFile.isOnline 		= true;
		    									...
		    									//Add the manifest file to the attachments array
		    									//
		    									if(manifestFile != null)
		    										{
		    											emailAttachments.push(manifestFile);
		    										}
					...
					//Email the results
					//
					var emailSubject 	= 'Carrier Integration End Of Day Processing';
					var emailBody 		= 'Please see the attached file(s) for details of the carrier integration end of day processing';
					
					try
						{
							//Send email to the nominated user
							//
    ...
    return {execute: executeEodCommit};
