    ...
    function createConsolidatedTenantChargesReport(reportDate)
		{
			...
			// start off the CSV
			var CSV = '"Quantity","Telephone","AccountCode","Product","Type","Due","UnitCost","Discount","Cost","LinkInvoiceYear","LinkInvoiceMonth"\r\n';
			...
			for (var i = 0; i < searchResults.length; i++)
				{
	    			...
	    			var quantity 	= searchResults[i].getValue(searchResults[i].columns[0]);
					var accountCode = searchResults[i].getValue(searchResults[i].columns[1]);
	    			var product 	= searchResults[i].getValue(searchResults[i].columns[2]);
	    			var unitCost 	= searchResults[i].getValue(searchResults[i].columns[3]);
	    			var cost 		= searchResults[i].getValue(searchResults[i].columns[4]);
					var year		= searchResults[i].getValue(searchResults[i].columns[5]);
					var month		= searchResults[i].getValue(searchResults[i].columns[6]);
				
	    			// add the service data record details to the CSV
	    			CSV += quantity + ',' + ',' + accountCode + ',' + product + ',' + ',' + ',' + unitCost + ',' + 0 + ',' + cost + ',' + year + ',' + month;
	    			CSV += '\r\n';
				}
