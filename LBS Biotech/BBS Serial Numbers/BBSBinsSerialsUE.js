					...
					for (var int = 1; int <= count; int++) 
						{
									  ...
									  for(var x = 1; x <=invcount ; x++) 
										  {
											  ...
											  var serialQty = rec.getLineItemValue('inventoryassignment', 'quantity', x);
											  ...
											  if(serialNumber != null && serialNumber != '')
												  {
													  ...
													  serials += serialNumber + '(' + serialQty + ')';
													  serialCount++;
												  }
										  }
								}
							...
							if(serialCount > 0)
								{
									thisRecord.setLineItemValue('item', 'custcol_serial_numbers', int, serials);
											...
											for (var int2 = 1; int2 <= soLines; int2++) 
												{
													...
													if(soLineNo == itemOrderLine)
														{
															salesOrderRecord.setLineItemValue('item', 'custcol_serial_numbers', int2, serials);
